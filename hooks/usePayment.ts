...
export function usePayment() {
    ...
    try {
      ...
      if (isSuccess) {
        const completedTransaction = {
          ...
          gatewayTransactionId: `gw_${Math.random().toString(36).substr(2, 12)}`,
        }
  ...
  const getRandomFailureReason = (methodType: string): string => {
    ...
    const methodSpecificReasons = {
      ...
      upi: ["UPI ID not found", "UPI app not responding", "Daily limit exceeded"],
    ...
    const reasons = [
      ...commonReasons,
      ...(methodSpecificReasons[methodType as keyof typeof methodSpecificReasons] || []),
    ]
  ...
  const retryPayment = async (): Promise<PaymentTransaction | null> => {
    ...
    return initiatePayment(currentTransaction.amount, currentTransaction.method, {})
  }
